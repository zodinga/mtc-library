	...
	<div class="col-lg-5">
		{{ Form::open(array('url'=>'idcard', 'method'=>'POST', 'class'=>'form-horizontal idcard-form', 'enctype'=>'multipart/form-data', 'autocomplete'=>'off')) }}
			...
			<div class="form-group {{($errors->has('st_blood_group'))?'has-error':''}}">
				...
				<div class="col-sm-7">
					{{ Form::select('st_blood_group', array('O+ve'=>'O+ve', 'A+ve'=>'A+ve', 'B+ve'=>'B+ve', 'AB+ve'=>'AB+ve','O'=>'O', 'A'=>'A', 'B'=>'B', 'AB'=>'AB'), Input::old('st_blood_group'), array('id'=>'staff_blood_group', 'placeholder'=>'', 'class'=>'form-control')) }}
				</div>
			</div>
...
$(function(){
	...
	$('#staff_date_of_birth').datepicker({
		format: 'dd.mm.yyyy',
		todayBtn: 'linked'
	}).on('changeDate', function(ev){
		$('#staff_idcard .dob').text($('#staff_date_of_birth').val());
		$('#staff_date_of_birth').datepicker('hide');
	});
